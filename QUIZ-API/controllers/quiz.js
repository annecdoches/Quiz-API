...
//CREATE: untuk menambahkan data kedalam tabel quiz exports.create = async (req, res) => {
...
exports.update = async (req, res) => {
  ...
  try{
    ...
    res.json({
      message: "Quizzes updated sucessfully.",
      data: quiz,
    });
  } catch (error) {
    res.status(500).json({
...
exports.delete = async (req, res) => {
  ...
  try{
    ...
    res.json({
      message: "Quizzes deleted sucessfully.",
    });
  } catch (error) {
    res.status(500).json({
      message: error.message || "Some error occurred while retrieving quiz",
...
exports.findOne = async (req, res) => {
  ...
  try{
    const quiz = await Quiz.findByPk(id, { rejectOnEmpty: true })
    res.json({
      message: `Quizzes retrieved sucessfully with id=${id}.`,
      data: quiz,
    });
  } catch (error) {
    res.status(500).json({
